...
y-key), see the output (*Array summaries setup: Creating ['q6.q6_grid'].*). 
...
Arrays can also be transposed ('@'/total as x-key and array as y-key). If they 
are not in the batch summary list before, they are automatically added and 
depending on the ``replace`` parameter only the transposed or both types of
summaries are added to the ``qp.Stack``:
