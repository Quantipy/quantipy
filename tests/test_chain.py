...
# @pytest.fixture(scope='class')
def unnamed_chain_for_structure():
    _, data = dataset()
    columns = ['record_number', 'age']
    _frame = data.head(1).loc[:, columns]
    _basic_chain = basic_chain()
    _basic_chain.add(_frame, DATA_KEY)
    return _basic_chain
...
def chain_for_structure():
    ...
    _basic_chain = basic_chain()
    _basic_chain.add(_frame, DATA_KEY, name='open')
    return _basic_chain

# @pytest.fixture(scope='function')
def chain_structure(unnamed, paint=False, sep=None):
    if unnamed:
        _chain_for_structure = unnamed_chain_for_structure()
    else:
        _chain_for_structure = chain_for_structure()
    if paint:
        _chain_for_structure.paint_all(sep=sep or '. ',
                                      text_key='en-GB',
                                      na_rep=parameters.AST)
    it = iter(_chain_for_structure)
    return next(it)
...
class TestChainConstructor:
    def test_init(self):
        _basic_chain = basic_chain()
        assert isinstance(_basic_chain.stack, qp.Stack)

    def test_str(self):
        _basic_chain = basic_chain()
        assert str(_basic_chain) == ""

    def test_repr(self):
        _basic_chain = basic_chain()
        assert repr(_basic_chain) == str(_basic_chain)

    def test_len(self):
        _basic_chain = basic_chain()
        assert len(_basic_chain) == 0

class TestChainExceptions:
    def test_get_non_existent_columns(self):
        expected = "Expecting ValueError"
        with pytest.raises(ValueError, message=expected) as excinfo:
            _basic_chain = basic_chain()
            _basic_chain.get(data_key=DATA_KEY,
...
class TestChainGet:
    ...
    def test_get_x_orientation(self):

        params_getx = [
            (['q5_1'], ['@', 'q4'], parameters.X1),
            (['q5_1'], ['@', 'q4 > gender'], parameters.X2),
            (['q5_1'], ['@', 'q4 > gender > Wave'], parameters.X3),
            (['q5_1'], ['@', 'q4 > gender > Wave', 'q5_1', 'q4 > gender'], parameters.X4)
        ]

        for params in params_getx:

            x, y, expected = params

            _stack = stack()

            chains = complex_chain(_stack, x, y, self._VIEWS, self._VIEW_KEYS, 'x',
    ...
    def test_sig_transformation_simple(self):
        _stack = stack()
        x, y = 'q5_1', ['@', 'gender', 'q4']
        chains = complex_chain(_stack, x, y, self._VIEWS, self._VIEW_SIG_KEYS,
    ...
    def test_annotations_fields(self):
        _stack = stack()
        x, y = 'q5_1', ['@', 'gender', 'q4']
        chains = complex_chain(_stack, x, y, self._VIEWS, self._VIEW_SIG_KEYS,
                               'x', incl_tests=True, incl_sum=True)
    ...
    def test_annotations_list_append(self):
        _stack = stack()
        x, y = 'q5_1', ['@', 'gender', 'q4']
        chains = complex_chain(_stack, x, y, self._VIEWS, self._VIEW_SIG_KEYS,
                               'x', incl_tests=True, incl_sum=True)
    ...
    def test_sig_transformation_large(self):
        pass
...
class TestChainAdd:
    def test_named(self):
        _chain = chain_structure(False)
        assert _chain.name == 'open'

    def test_str(self):
        for params_structure in PARAMS_STRUCTURE:
            paint, columns, values, repaint_columns, _ = params_structure

            _chain = chain_structure(False, paint=paint)
