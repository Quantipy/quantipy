...
class Stack(defaultdict):
    ...
    def add_data(self, data_key, data=None, meta=None, ):
        ...
        * data and meta
        ...
        if data_key in self.keys():
            ...
            print(warning_msg.format(data_key))
    ...
    def variable_types(self, data_key, only_type=None, verbose=True):
        ...
        if self[data_key].meta['columns'] is None:
            ...
            if not_found and verbose:
                print('{} not found in meta file. Ignored.'.format(not_found))
            if only_type:
    ...
    def get_chain(self, *args, **kwargs):
        ...
        def _get_chain(name=None, data_keys=None, filters=None, x=None, y=None,
            ...
            A chain is a one-to-one or one-to-many relation with an orientation
            thatdefines from which axis (x or y) it is build.
...
#                 xs = self[dk][fk]
    ...
    def add_nets(self, on_vars, net_map, expand=None, calc=None, rebase=None,
        ...
        for dk in self.keys():
            ...
            if recode:
                ds = ds = DataSet(dk, dimensions_comp=meta['info'].get('dimensions_comp'))
    ...
    def add_stats(self, on_vars, stats=['mean'], other_source=None, rescale=None,
        ...
        for dk in self.keys():
            ...
            for v in on_vars:
                ...
                ds = DataSet(dk, dimensions_comp=meta['info'].get('dimensions_comp'))
                ds.from_stack(self, dk)
                if not other_source:
        ...
        for dk in self.keys():
            ...
            for batch_name in _batches:
                ...
                if verbose and levels:
                    print('\n')
        if verbose:
            print('Sig-Tests: {}'.format(time.time()-start))
        return None
