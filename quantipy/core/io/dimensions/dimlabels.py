...
class DimLabels():
    ...
    def _lang_ltype_from_label(self, replace=True):
        ...
        for lab in self.labels:
            if lab.language not in self.incl_languages:
                self.incl_languages.append(lab.language)
            if lab.labeltype and lab.labeltype not in self.incl_labeltypes:
    ...
    def _label_exists(self, label):
        return any(
            d_l.language == label.language and d_l.labeltype == label.labeltype
