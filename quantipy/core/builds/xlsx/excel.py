...
from ...tools.qp_decorators import lazy_property
...
class Excel(Workbook):
    ...
    **formats : ``dict``
    ...
    Notes
    -----
    TBD -> take over notes from previous sandbox version into readthedocs
    """
    ...
    def image(self):
        if self._image:
            ...
            image.thumbnail(
                self._image.get(
                    'img_size', self.sheet_properties['img_size']),
                Image.ANTIALIAS)
    ...
    def _get_annotations(self, annotations, sheet_name):
        err = (
            '"annotations" passed to Excel() must either be a list, for all '
            'sheets, or a dict, key=sheet name : value=list of annotations')
        if self.annotations:
...
class _Sheet(Worksheet):
    ...
    def write_chains(self):
        ...
        if self.excel.details and all(c.structure is None
    ...
    def set_row(self, row, height, label=None, font_name=None, font_size=None):
        ...
        if isinstance(label, basestring):
            ...
            if font_size is not None:
                def_size = self.excel._formats.default_attributes['font_size']
                fact = float(font_size) / float(def_size)
            else:
                fact = 1
            ...
            size_col = self._size_col(self.start_column)
            check_against = int(size_col / units_to_pixels)
            if (dimensions[0] * fact) >= check_against:
                # text too long
                return
...
class _Box(object):
    ...
    def _write_data(self):
        ...
        def __write_label(index):
            ...
            if index == 'index':
                le = self.sheet._row + left
                r = self.sheet._row + right
            else:
                le = self.sheet._column + left + offset
                r = self.sheet._column + right + offset

            if merge_columns and (index == 'column'):
                if left == right:
                    merge_range(fixed, le, fixed + 1, le, label, format_)
                else:
                    for column in xrange(le, r + 1):
                        merge_range(
                            fixed, column, fixed + 1, column, label, format_)
            else:
                if left == right:
                    if index == 'index':
                        write(le, fixed, label, format_)
                    else:
                        write(fixed, le, label, format_)
                else:
                    if index == 'index':
                        merge_range(le, fixed, r, fixed, label, format_)
                    else:
                        merge_range(fixed, le, fixed, r, label, format_)
        ...
        for level_id in xrange(nlevels):
            ...
            while True:
                ...
                if level_id == 0:
                    if left == right:
                        ...
                        l_label = self.columns.get_level_values(level)[left]
                        if l_label in ['Total', 'Gesamt']:
                ...
                if left not in self.single_columns:
                    if group_sizes and not is_values:
                        ...
                        if write_labels:
                            merge_range(
                                row - int(arr_style_0), column + left,
            ...
            if not has_tests or (((level_id + 1) != nlevels) and has_tests):
                if (row % 2) == 0:
                    ...
                    self.sheet.set_row(
                        row - int(arr_style_0), self.sheet.y_row_height)
    ...
    def _write_rows(self):
        ...
        if self.chain._is_mask_item:
            label_f = self.formats['mask_label']
        else:
            label_f = self.formats['label']
        write(self.sheet._row, column, levels(0).unique().values[0], label_f)
        self._format_row(label_f)
        self.sheet._row += 1
        ...
        for data in flat:
            ...
            if rel_y == 0:
                if data == '' or name in ['propstest', 'meanstest']:
                    view_border = False
                else:
                    view_border = True
            ...
            if any(_ in str(cell_data) for _ in ("'@L'", "'@H'")):
                ...
                if arrow_format is _None:
                    arrow_format = self._format_x(
                        name, rel_x, rel_y, row_max, x_contents.get('dummy'),
                        use_bg, view_border, border_from,
