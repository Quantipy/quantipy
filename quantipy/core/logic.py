    ...
    if isinstance(values, (list, tuple)):
        for value in values:
            if not isinstance(value, int):
                raise TypeError(
                    "The values given to {}() are not correctly "
                    "typed. Expected list of <int>, found a {}.".format(
                        func_name, type(value))
                )
    else:
        raise TypeError(
            "The values given to {}() must be given as a list. "
            "Expected a <list>, found a {}".format(func_name, type(values))
        )


def verify_logic_series(series, func_name):
    """
    Verifies that the series given is a compatible type:
    ...
    if series.dtype not in ['object', 'int64', 'float64']:
        raise TypeError(
            "The series given to {}() must be a supported dtype. "
            "Expected 'object', 'int64' or 'float64', found a '{}'.".format(
...
def verify_count_responses(responses, func_name):
    ...
    if not len(responses) in [1, 2, 3]:
        raise IndexError(
            "The responses list given to {}() must have "
            ...
            "Found {}.".format(func_name, responses)
        )
    ...
    if isinstance(responses[0], tuple):
        if not responses[0][0] in [_is_lt, _is_le, _is_eq, _is_ne, _is_ge,
                                   _is_gt]:
            msg = (
                "The binary function given to {}() is not recognized. "
                "Found {}.")
            raise TypeError(msg.format(func_name, responses))
        if not isinstance(responses[0][1], int):
            msg = (
                "The numerator given to {}() is incorrectly typed. It must be "
                "<int>. Found {}.")
            raise TypeError(msg.format(func_name, responses))
        ...
        if len(responses) == 2:
            for value in responses[1]:
                if not isinstance(value, int):
                    msg = (
                        "The values subset given to {}() are not correctly "
                        "typed. Each value must be <int>. Found {}.")
                    raise TypeError(msg.format(func_name, value))
        return responses

    if len(responses) == 1:
        if not isinstance(responses[0], int):
            msg = (
                "The count target given to {}() is incorrectly typed. It must "
                "be <int>. Found {}.")
            raise TypeError(msg.format(func_name, responses))
        return responses
    ...
    if len(responses) in [2, 3]:
        for value in responses[:2]:
            if not isinstance(value, int):
                msg = (
                    "The values subset given to {}() are not correctly typed. "
                    "Each value must be <int>. Found {}.")
                raise TypeError(msg.format(func_name, value))
...
def verify_numeric(value, func_name):
    ...
    try:
        ...
        msg = "The value given to is_{}() must be numeric. Found {}.".format(
            func_name, type(value))
...
def _any_all(series, values, func_name, exclusive=False, _not=False):
    """
    Returns the index of rows from series containing any/all of the given
    values as requested by func_name.
    ...
    if series.dtype == 'object':
        ...
        if not cols:
            ...
            if exclusive:
                other_cols = [
                    col for col in dummies.columns if col not in values]
                o_dummies = dummies[other_cols]
                o_dummies = o_dummies[(o_dummies.T == 1).any()]
        ...
        if 'any' in func_name:
            ...
            if exclusive:
                if _not:
                    exclusive_idx = o_dummies.index.difference(dummies.index)
                    return exclusive_idx
                else:
                    exclusive_idx = dummies.index.difference(o_dummies.index)
        ...
        raise TypeError(
            "The dtype '{}' of series is incompatible with has_{}()".format(
                series.dtype, func_name))


def has_any(values, exclusive=False):
    """
    Convenience for managing 'any' part of the 'logic' instructions provided in
    a freq method's kwargs.
...
def _has_any(series, values, exclusive=False):
    """
    Returns the index of rows from series containing any of the given values.
...
def has_all(values, exclusive=False):
    """
    Convenience for managing 'all' part of the 'logic' instructions provided in
    a freq method's kwargs.
...
def _has_all(series, values, exclusive=False):
    """
    Returns the index of rows from series containing all of the given values.
...
def has_count(responses, exclusive=False):
    """
    Convenience for managing the 'count of responses' part of the 'logic'
...
def _has_count(series, responses, exclusive=False):
    """
    Returns the index of rows from series containing the targeted number or
    range of responses.
...
def is_lt(value):
    """
    Convenience for managing 'less than' part of the 'logic' instructions
...
def is_le(value):
    """
    Convenience for managing 'less than or equal to' part of the 'logic'
    instructions provided in a freq method's kwargs.
...
def is_eq(value):
    """
    Convenience for managing 'equal to' part of the 'logic' instructions
    provided in a freq method's kwargs.
...
def is_ne(value):
    """
    Convenience for managing 'not equaL to' part of the 'logic' instructions
    provided in a freq method's kwargs.
...
def is_ge(value):
    """
    Convenience for managing 'greater than or equal to' part of the 'logic'
    instructions provided in a freq method's kwargs.
...
def is_gt(value):
    """
    Convenience for managing 'greater than' part of the 'logic' instructions
    provided in a freq method's kwargs.
    ...
    -------
    _lt : function
        The function that will be used to evaluate the 'greater than' condition
...
def union(logic_list):
    """
    Convenience for managing union logic provided in a freq method's kwargs.

    Parameters
    ----------
    logic_list : list
        The list of logical conditions the be chained with a union operation.
    ...
    logic_list : list
        The list of logical conditions to be chained with a union operation.
    """
...
def _union(idxs):
    ...
    -------
    idx : pandas.Index
        The result of the chained union of the indexes given.
    """
    idx = idxs[0]
...
def intersection(logic_list):
    """
    Convenience for managing intersection logic provided in a freq method's
    kwargs.

    Parameters
    ----------
    logic_list : list
        The list of logical conditions the be chained with a intersection
        operation.
    ...
    logic_list : list
        The list of logical conditions to be chained with a intersection
        operation.
    """
...
def _intersection(idxs):
    ...
    -------
    idx : pandas.Index
        The result of the chained intersection of the indexes given.
    """
    idx = idxs[0]
    for idx_part in idxs[1:]:
...
def difference(logic_list):
    """
    Convenience for managing difference logic provided in a freq method's
    kwargs.
...
def _difference(idxs):
    ...
    -------
    idx : pandas.Index
        The result of the chained difference of the indexes given.
    """
    idx = idxs[0]
    for idx_part in idxs[1:]:
...
def symmetric_difference(logic_list):
    """
    Convenience for managing symmetrical difference logic provided in a freq
    method's kwargs.
    ...
    _symmetrical difference : function
        The function that will be used to evaluate the symmetrical difference
...
def _symmetric_difference(idxs):
    ...
    -------
    idx : pandas.Index
        The result of the chained symmetrical difference of the indexes given.
    """
    idx = idxs[0]
    for idx_part in idxs[1:]:
        if pd.__version__ == '0.19.2':
...
def apply_set_theory(func, series, logic_list, data):
    """
    Returns the result of chained binary operations defined in logic_list using
    the given function.
    ...
    data : pandas.DataFrame
        The source data from which wildcard variable references may be drawn.
    ...
    vkey : str
        The relationship-part of the view key that represents this logical
        block.
    """
    idxs = []
    vkeys = []
    ...
    vkey = '({})'.format(__set_symbol__[func].join(vkeys))
    return idx, vkey
...
def get_logic_key_chunk(func, values, exclusive=False):
    ...
    if func in [_is_lt, _is_le, _is_eq, _is_ne, _is_ge, _is_gt]:
        chunk = '({}{})'.format(__op_symbol__[func], values)
        ...
        if isinstance(_min, tuple):
            ...
            op_func = _min[0]
            numerator = _min[1]
            min_max = '{}{}'.format(__op_symbol__[op_func], numerator)
            if len(values) == 2:
                values = values[1]
                values = [str(v) for v in values]
            else:
        ...
        if _max is not None:
            if _min == _max:
                ...
                if op_func is None:
                    min_max = '{}-{}'.format(_min, _max)

        if values is None:
            chunk = '{}{{}}'.format(_not, min_max)
        else:
            chunk = '{}({}){}{{}}'.format(
                excl, ','.join(values), _not, min_max)
...
def get_logic_key(logic, data=None):
    """
    Uses the given complex logic block to return the matched view key.
